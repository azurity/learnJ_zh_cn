...
## 3.7 函数的复合(Composition of Functions)
...
一个单独的平方和函数也可以写成`sum`和`square`的复合：
...
符号`@:`(花A 冒号)被叫做“复合”算符。效果是，如果`f`和`g`都是动词，对于任意参数`y`
...
可能你会问为什么写`(f @: g)`而不是直接写`(f g)`来表示复合。原因是`(f g)`表示其他内容，接下来会详细说明。
另一个复合的例子。华氏度可以通过函数`s`（减 32）和`m`（乘以`5%9`）的复合来计算的得到摄氏度。
...
为了清楚，这些表达式由显式命名的函数复合。当然也可以直接复合表达式：
...
之前的例子展示了单目函数与单目函数的复合。接下来的的例子展示单目函数与双目函数的复合。方式如下：
...
一个三动词的孤立序列被叫做分叉。运作方式如下：`f`是一个单目函数，`g`是一个双目函数，`h`是一个单目函数对于任意参数y， 
...
另一个关于分叉的例子。求一个数列里数字的范围。可以通过分叉`smallest , largest`来实现。（中间的动词是逗号）
...
钩子和分叉都是动词序列，也就是被称为“train”的动词。更多与train相关的内容，详见[第九章](chapter9.md)。
...
为了制作显示在底部的那一行，我们定义了分叉`br`来将数据和计算结果连在一起： 
...
回顾一下，我们看到了一个小的J语言程序，包含了几种J语言中的特性：结合，复合，钩子和分叉。和许多J语言程序一样，这只是它众多可能的编写方式之一。

在这章中，我们首先介绍了定义函数的方式。有两种不同的函数：动词，算符。到目前为止，我们只是介绍了定义动词。下一章，我们将会介绍另一种定义动词的方式，在[第十三章](chapter13.md)将会介绍如何定义算子。 